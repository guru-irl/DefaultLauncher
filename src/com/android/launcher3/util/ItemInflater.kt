...
class ItemInflater<T>(
        ...
        try {
            ...
            if (type == WidgetInflater.TYPE_DELETE) {
                android.util.Log.e("ItemInflater",
                    "TYPE_DELETE for widget id=${item.appWidgetId} — treating as " +
                    "TYPE_MISSING (not deleting). docs/changes/080")
            }
    ...
    private fun inflateWidgetStack(info: WidgetStackInfo, writer: ModelWriter): View {
        ...
        for (child in info.getContents()) {
            ...
            // TYPE_DELETE no longer returned by WidgetInflater; guard defensively.
            if (type == WidgetInflater.TYPE_DELETE) {
                android.util.Log.e("ItemInflater",
            ...
            if (type == WidgetInflater.TYPE_MISSING || type == WidgetInflater.TYPE_DELETE) {
                // Provider absent — placeholder slot in the stack. Child stays in DB.
                ...
                val unavailView = UnavailableWidgetView(context, widgetInfo) {
                    ...
                    // If stack now empty, remove the parent container too.
        ...
        // With TYPE_MISSING all valid children are in inflatedIds, so only
        // truly non-widget-type entries are removed here.
