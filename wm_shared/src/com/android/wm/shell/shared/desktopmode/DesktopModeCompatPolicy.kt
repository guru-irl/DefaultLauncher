...
import com.android.wm.shell.shared.compat.HiddenApiCompat
...
class DesktopModeCompatPolicy(private val context: Context) {
    ...
    private val systemUiPackage: String = try {
        val resId = Class.forName("com.android.internal.R\$string")
            .getField("config_systemUi").getInt(null)
    ...
    private val defaultHomePackage: String?
        get() = defaultHomePackageSupplier?.get()
            ?: HiddenApiCompat.getHomeActivities(pkgManager)?.packageName
    ...
    fun isTopActivityExemptFromDesktopWindowing(task: TaskInfo) =
        isTopActivityExemptFromDesktopWindowing(task.baseActivity?.packageName,
            task.numActivities, HiddenApiCompat.isTopActivityNoDisplay(task),
            HiddenApiCompat.isActivityStackTransparent(task),
            HiddenApiCompat.getUserId(task))
    ...
    fun isTopActivityExemptFromDesktopWindowing(
        ...
        val desktopModeFlagsEnabled = try {
            val clazz = Class.forName("android.window.DesktopModeFlags")
            val field = clazz.getField("ENABLE_DESKTOP_WINDOWING_MODALS_POLICY")
            val flag = field.get(null)
            flag?.javaClass?.getMethod("isTrue")?.invoke(flag) as? Boolean ?: false
        } catch (_: Exception) { false }

        return desktopModeFlagsEnabled &&
    ...
    fun shouldExcludeCaptionFromAppBounds(taskInfo: TaskInfo): Boolean {
        val topActivityInfo = HiddenApiCompat.getTopActivityInfo(taskInfo) ?: return false
        val isResizeable = HiddenApiCompat.isResizeable(taskInfo)
        val appCompatTaskInfo = HiddenApiCompat.getAppCompatTaskInfo(taskInfo)
        val hasOptOut = HiddenApiCompat.hasOptOutEdgeToEdge(appCompatTaskInfo)
        return try {
            val utilsClass = Class.forName("com.android.internal.policy.DesktopModeCompatUtils")
            utilsClass.getMethod(
                "shouldExcludeCaptionFromAppBounds",
                android.content.pm.ActivityInfo::class.java,
                Boolean::class.javaPrimitiveType,
                Boolean::class.javaPrimitiveType
            ).invoke(null, topActivityInfo, isResizeable, hasOptOut) as? Boolean ?: false
        } catch (_: Exception) { false }
    }
    ...
    fun isTransparentTask(task: TaskInfo): Boolean =
        isTransparentTask(HiddenApiCompat.isActivityStackTransparent(task), task.numActivities)
    ...
    private fun hasFullscreenTransparentPermission(packageName: String?, userId: Int): Boolean {
        val flagEnabled = try {
            val clazz = Class.forName("android.window.DesktopModeFlags")
            val field = clazz.getField("ENABLE_MODALS_FULLSCREEN_WITH_PERMISSIONS")
            val flag = field.get(null)
            flag?.javaClass?.getMethod("isTrue")?.invoke(flag) as? Boolean ?: false
        } catch (_: Exception) { false }
        ...
        if (flagEnabled) {
            ...
            return packageInfoCache.getOrPut("$userId@$packageName") {
                try {
                    val packageInfo = HiddenApiCompat.getPackageInfoAsUser(
                        pkgManager,
                        packageName,
