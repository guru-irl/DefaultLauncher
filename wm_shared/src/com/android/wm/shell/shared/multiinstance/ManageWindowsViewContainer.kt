...
abstract class ManageWindowsViewContainer(
    ...
    fun createMenu(snapshotList: List<Pair<Int, Any?>>,
             onIconClickListener: ((Int) -> Unit),
             onOutsideClickListener: (() -> Unit)): ManageWindowsView {
        val bitmapList = snapshotList
            .filter { it.second != null }
            .map { (index, snapshot) ->
                val hardwareBuffer = snapshot!!.javaClass.getMethod("getHardwareBuffer")
                    .invoke(snapshot) as? android.hardware.HardwareBuffer
                val colorSpace = snapshot.javaClass.getMethod("getColorSpace")
                    .invoke(snapshot) as? android.graphics.ColorSpace
                index to (if (hardwareBuffer != null) Bitmap.wrapHardwareBuffer(hardwareBuffer, colorSpace) else null)
            }
    ...
    class ManageWindowsView(
        ...
        fun generateIconViews(
            ...
            for ((iconCount, taskInfoSnapshotPair) in snapshotList.withIndex()) {
                ...
                HiddenApiCompat.setCornerRadius(appSnapshotButton, iconRadius)
                ...
                rowLayout?.post {
                    HiddenApiCompat.attachAndQueueBufferWithColorSpace(
